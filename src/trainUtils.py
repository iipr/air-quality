...
class DataGenerator(Sequence):
    ...
    def __init__(self, dataset_d, extent, n_x, n_y, shift, batch_size,
                 group='/gases', idx_v=None, go=False):
        '''Initialization of the generator object
        
        Keyword arguments:
        dataset_d -- dictionary with dataset paths (current keys: sources, [weather], [holidays])
        ...
        if self.holidays is not None:
            self.holidays = pd.read_csv(self.holidays, parse_dates=[1])
        self.group = group
        ...
        self.n_y = n_y
        self.shift = shift
        self.idx_v = idx_v if idx_v is not None else slice(None)
        ...
        with tb.open_file(self.time_series, mode='r') as h5_file:
            aux_shape = list(h5_file.get_node(self.group).shape[1:])
            if idx_v is not None:
                aux_shape[-1] = len(self.idx_v)
            self.X_shape = np.array([self.n_x, *aux_shape]) #X_shape
            self.y_shape = np.array([self.n_y, *aux_shape]) #y_shape
    ...
    def __getitem__(self, batch_n):
        ...
        X_ts = np.empty((self.curr_bs, *self.X_shape))
        y = np.empty((self.curr_bs, *self.y_shape))
        #for idx_dset, ts_dataset in enumerate(self.time_series):
        X_ts, y = self.__data_generation(self.time_series, *x_extent, group=self.group, idx_v=self.idx_v)
        #print(X_ts.shape)
        #print(X_ts.nbytes)
        ...
        if self.weather is not None:
            ...
            X['weather'] = self.__data_generation(self.weather, *x_extent, group='/weather',
                                                  idx_v=slice(None), n_repeat=n_repeat)[0][:, :, 1:]
            #print(X['weather'].shape)
            #print(X['weather'])
            #return
        # Calendar aware:
        if self.holidays is not None:
    ...
    def __idx_to_datetime(self, idx, freq, year=2010):
    ...
    def __get_input_times(self, x_l, x_r, freq='15min', week_d=dict(zip(range(7), [0]*5 + [1]*2))):
        '''
        ...
        time_ret = np.stack([np.sin(times_s * 2 * np.pi / day),
                             ...
                             times.weekday.map(week_d),
                             times.normalize().isin(self.holidays.date).astype(int)],
                            axis=1)
        return time_ret

    def __data_generation(self, dset_path, x_l, x_r, group, idx_v, n_repeat=1):
        ...
        with tb.open_file(dset_path, mode='r') as h5_file:
            X_slc = h5_file.get_node(group)[x_l:x_r][..., idx_v]
            ...
            y_slc = h5_file.get_node(group)[y_l:y_r][..., idx_v]
